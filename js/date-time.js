class DateTime {
	...
	_setTime() {
		...
		if (hour >= 6 && hour < 12) {
			...
			greeterSuffix = 'tardes';
		} else {
			greeterSuffix = 'noches';
		}
		...
		this._greeterDate.innerText = ` ${this._daysArr[date.getDay()]} ${this._getDayOrdinal(this._appendZero(date.getDate()))} de ` +
			`${this._monthsArr[date.getMonth()]}`;

		if(`${greeterSuffix}` == "dìas"){
			this._greeterMessage.innerText = `Buenos ${greeterSuffix}!`;
		}else{
			this._greeterMessage.innerText = `Buenas ${greeterSuffix}!`;
		}
